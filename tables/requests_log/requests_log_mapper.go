...
func (m *RequestsLogMapper) Map(_ context.Context, a any, _ ...mappers.MapOption[*RequestsLog]) (*RequestsLog, error) {
	...
	case logging.Entry:
		project := strings.Split(v.LogName, "/")[1]
		preserverLogName := v.LogName
		v.LogName = ""
		protoLogEntry, err := logging.ToLogEntry(v, project)
		if err != nil {
			return nil, fmt.Errorf("error converting log entry to loggingpb.LogEntry: %w", err)
		}
		protoLogEntry.LogName = preserverLogName
		return mapFromSDKType(protoLogEntry)
	case []byte:
