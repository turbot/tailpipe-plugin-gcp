...
type AuditDataAccessLogTable struct {
	table.TableImpl[*rows.AuditActivityLog, *AuditDataAccessLogTableConfig, *config.GcpConnection]
}
...
func (c *AuditDataAccessLogTable) EnrichRow(row *rows.AuditActivityLog, sourceEnrichmentFields *enrichment.CommonFields) (*rows.AuditActivityLog, error) {
